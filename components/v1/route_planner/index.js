...
const RoutePlannerController = require('./controller');
const controller = new RoutePlannerController();
...
router.post('/', (req, res) => { 
    controller.getDirections(req.body).then(response => {
...
router.post('/locationexists', (req, res) => {
    console.log(req.body);
    controller.locationExists(req.body).then(response => {
        // res.status(response.status_code).send(Misc.formattedResponse(response))
        res.status(200).send(Misc.formattedResponse(response));
    }).catch(error => {
        // res.status(500).send(Misc.formattedResponse(new Response('error', Errors.server_error, error.message, 500)));
        console.log(error);
        res.status(500).send(Misc.formattedResponse(new Response('error', Errors.server_error,error.message, 500)));
